...
Every payload's fields are broken down where its feature lives, rather than repeated here: [row selection](/docs/react/row-selection#payload-shape) for `RowSelectionChangeEvent`, [column selection](/docs/react/column-selection#payload-shapes) for `ColumnSelectEvent`/`ColumnsSelectEvent`/`ColumnSelectionChangeEvent`, and [cell selection](/docs/react/cell-selection#payload-shapes) for `CellSelectEvent`/`CellSelectionChangeEvent`.
